window.addEventListener(
  ...
  function(e) {
    ...
    var apiUrl = "//api.22family.com/blog";
    var info = [];
    var timeArr = [];
    var showLrcArr = [];
    var currentLrc = 0;
    ...
    function update() {
      ...
      tolSecond < 10
        ? (totalDom.innerHTML =
            Math.floor(audioDom.duration / 60) + ":0" + tolSecond)
        : (totalDom.innerHTML =
            Math.floor(audioDom.duration / 60) + ":" + tolSecond);
      //显示时间与音乐同步
    ...
    function xunran(colse) {
      ...
      containerDom.style.background =
        "url(" + image_api_url + musicList[num].imgUrl + ") 0 0 /cover";
    ...
    function lookLrc() {
      ...
      var m =
        Math.floor(timeArr[currentLrc] / 60) < 10
          ? "0" + Math.floor(timeArr[currentLrc] / 60)
          : Math.floor(timeArr[currentLrc] / 60);
      var s =
        Math.floor(timeArr[currentLrc] % 60) < 10
          ? "0" + Math.floor(timeArr[currentLrc] % 60)
          : Math.floor(timeArr[currentLrc] % 60);
    ...
    function ajaxGetHTML(music, callback) {
      showLrcArr = [];
      timeArr = [];
      var url = apiUrl + "/pc/lrc?music=" + music;
